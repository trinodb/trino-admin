...
from trinoadmin.trinoclient import URL_TIMEOUT_MS, TrinoClient
...
class TestTrinoClient(BaseTestCase):
    def test_no_sql(self, mock_presto_config):
        client = TrinoClient('any_host', 'any_user')
        self.assertRaisesRegexp(InvalidArgumentError,
                                "SQL query missing",
                                client.run_sql, "", )

    def test_no_server(self, mock_presto_config):
        client = TrinoClient("", 'any_user')
        self.assertRaisesRegexp(InvalidArgumentError,
                                "Server IP missing",
                                client.run_sql, "any_sql")

    def test_no_user(self, mock_presto_config):
        client = TrinoClient('any_host', "")
        self.assertRaisesRegexp(InvalidArgumentError,
                                "Username missing",
                                client.run_sql, "any_sql")

    @patch.object(TrinoClient, '_create_auth_headers', return_value={'X-Presto-Internal-Bearer': 'any_bearer'})
